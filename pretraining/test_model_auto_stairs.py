...
def text_to_distance(input_text, num_stairs, target_distance, h1, h2):
    print(input_text)
    ...
    distance, t = utils.calculate_reward_stairs(np.array(output_sequence, dtype=np.float32), len(grid), num_stairs, target_distance, h1, h2)
...
model.load_state_dict(torch.load("./all_model/obj23.pth"))
...
#prompt = f"<|endoftext|>Please generate a robot design capable of walking from left to right down a stair: <|endoftext|>"
prompt = f"<|endoftext|>Please generate a robot design capable of walking from left to right down a stair of height at least {height}. The robot has to move at least {target_distance} distance to get down:<|endoftext|>"
#prompt = f"<|endoftext|>Please generate a robot design capable of walking from left to right down a stair of height at least {height} and then a stair of height at least {height2}:<|endoftext|>"
#prompt = f"<|endoftext|>Please generate a robot design capable of walking from left to right down a stair, using at most {max_blocks}. The robot has to move at least {target_distance} distance to get down:<|endoftext|>"
#prompt = f"<|endoftext|>Please generate a robot design capable of walking from left to right down a stair of height at least {height} and then another stair {height2}, using at most {max_blocks}. The robot has to move at least {target_distance} distance to get down:<|endoftext|>"
...
for i in range(n_generate):
    ...
    distance = 0
    output_sequence, distance, t = text_to_distance(output_string[0], 2, target_distance, height, height2)
    ...
    if distance > target_distance:
    #if num_blocks <= max_blocks and distance > target_distance:
